...
void AddMobilePostProcessingPasses(FRDGBuilder& GraphBuilder, FScene* Scene, const FViewInfo& View, const FMobilePostProcessingInputs& Inputs, FInstanceCullingManager& InstanceCullingManager)
{
	...
	if (bIsPostProcessingEnabled)
	{
		...
		PassSequence.SetEnabled(EPass::Custom, Scene->CustomPassData.bEnabled);
		...
		// custom pass
		if (PassSequence.IsEnabled(EPass::Custom))
		{
			PassSequence.AcceptPass(EPass::Custom);
			
			FRDGTextureDesc TextureDesc = FRDGTextureDesc::Create2D(SceneColor.Texture->Desc.Extent, PF_FloatRGBA, FClearValueBinding::Black, TexCreate_ShaderResource | TexCreate_RenderTargetable);
			FScreenPassRenderTarget TextureOutput = FScreenPassRenderTarget(GraphBuilder.CreateTexture(TextureDesc, TEXT("DirectionalBlurring")), SceneColor.ViewRect, ERenderTargetLoadAction::EClear);

			FCustomPS::FParameters* CustomPassParameters = GraphBuilder.AllocParameters<FCustomPS::FParameters>();
			CustomPassParameters->SceneColorTexture = SceneColor.Texture;
			CustomPassParameters->SceneColorTextureSampler = TStaticSamplerState<SF_Bilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI();
			CustomPassParameters->RenderTargets[0] = TextureOutput.GetRenderTargetBinding();

			TShaderMapRef<FCustomPS> CustomShader(View.ShaderMap);

			const FScreenPassTextureViewport InputViewport(SceneColor);
			const FScreenPassTextureViewport OutputViewport(TextureOutput);

			AddDrawScreenPass(GraphBuilder, RDG_EVENT_NAME("DirectionalBlurring"), View, OutputViewport, InputViewport, CustomShader, CustomPassParameters);

			SceneColor = TextureOutput;
		}
		//
